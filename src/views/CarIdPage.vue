...
const likes = ref(parseInt(localStorage.getItem(`likes_${route.params.id}`)) || 0);
const dislikes = ref(parseInt(localStorage.getItem(`dislikes_${route.params.id}`)) || 0);

const increase = () => {
  likes.value++;
  localStorage.setItem(`likes_${route.params.id}`, likes.value.toString());
};

const decrease = () => {
  dislikes.value++;
  localStorage.setItem(`dislikes_${route.params.id}`, dislikes.value.toString());
};
